...
export const Route = createFileRoute("/messages/$slug/$templateId")({
...
export function templateQueryOptions(templateId: string) {
...
function Page({
	...
	const saveTemplate = useSaveTemplate(templateId, {
		onSuccess: () => {
			const title = editedTemplate?.title;
			setEditedTemplate(null);
			if (searchParams.new) {
				navigate({ to: "/messages/$slug/$templateId", params: { templateId, slug: slugify(title ?? "") } });
			}
		},
	});
									...
									{session?.user && (
										<Label className="flex gap-1 items-center">
											<Link
												...
												params={{ templateId: uuidv7(), slug: "new-template" }}
												search={{
